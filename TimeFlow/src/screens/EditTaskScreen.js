...
export default function EditTaskScreen({ route, navigation }) {
  ...
  const initialStart = parseTime(taskData.date, taskData.startTime);
  const initialEnd = new Date(
    initialStart.getTime() + (taskData.duration || 60) * 60000,
  );

  // Стейт променливи
  const [title, setTitle] = useState(taskData.title);
  const [category, setCategory] = useState(taskData.category);
  const [date, setDate] = useState(new Date(taskData.date));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert("Грешка 😬", "Не успях да запазя промените.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Редактиране</Text>
          <View style={{ width: 44 }} />
        </View>
